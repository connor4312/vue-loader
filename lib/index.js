...
module.exports = function (source) {
  ...
  const stringifyRequest = r => {
    console.log(JSON.stringify({ r, resourcePath, rootContext }) + '\r\n\r\n')
    return loaderUtils.stringifyRequest(loaderContext, r)
  }

  const stringifyInternalRequest = r => {
    const relative = path.relative(rootContext, r)
    const internal = path.join(rootContext, '__vue__', relative)
    return stringifyRequest(internal)
  }
  ...
  if (descriptor.template) {
    ...
    const request = templateRequest = stringifyInternalRequest(src + query)
  ...
  if (descriptor.script) {
    ...
    const request = stringifyInternalRequest(src + query)
    scriptImport = (
